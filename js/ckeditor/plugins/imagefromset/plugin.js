...
( function() {
	...
	CKEDITOR.plugins.add( 'imagefromset', {
		...
		init: function( editor ) {
			...
			// when we'll rename imagefromset to image.
			...
			// names (sic!), this hack will be necessary as long as imagefromset is not named
			// Image. And since imagefromset will never be Image, for sure some filebrowser logic
			// got to be refined.
			...
			CKEDITOR.dialog.add( 'imagefromset', this.path + 'dialogs/imagefromset.js' );
		},
	...
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	// Non-captioned widget (config.imagefromset_alignClasses defined)
	// 		┌──────┬───────────────────────────────┬─────────────────────────────┐
	// 		│Align │Internal form                  │Data                         │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│none  │<wrapper>                      │<img />                      │
	...
	// 		└──────┴────────────────────────────────────────┴────────────────────────────────────────┘
	//
	// Captioned widget (config.imagefromset_alignClasses defined)
	// 		┌──────┬────────────────────────────────────────┬────────────────────────────────────────┐
	// 		│Align │Internal form                           │Data                                    │
	// 		├──────┼────────────────────────────────────────┼────────────────────────────────────────┤
	// 		│none  │<wrapper>                               │<figure />                              │
	...
	function widgetDef( editor ) {
		var alignClasses = editor.config.imagefromset_alignClasses,
			captionedClass = editor.config.imagefromset_captionedClass;
		...
		return {
			...
			init: function() {
				var helpers = CKEDITOR.plugins.imagefromset,
			...
			// Overrides default method to handle internal mutability of imagefromset.
			...
			getClasses: ( function() {
				...
				return function() {
					...
					// Neither config.imagefromset_captionedClass nor config.imagefromset_alignClasses
	...
	CKEDITOR.plugins.imagefromset = {
		stateShifter: function( editor ) {
			// Tag name used for centering non-captioned widgets.
			var doc = editor.document,
				alignClasses = editor.config.imagefromset_alignClasses,
				captionedClass = editor.config.imagefromset_captionedClass,
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: editor.lang.imagefromset.captionPlaceholder
						} ), doc );
	...
	// <figure class="{config.imagefromset_captionedClass}"> elements.
	//
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			// inline styles or classes (imagefromset_alignClasses).
	...
	function centerWrapperChecker( editor ) {
		var captionedClass = editor.config.imagefromset_captionedClass,
			alignClasses = editor.config.imagefromset_alignClasses,
		...
		return function( el ) {
			...
			// Centering wrapper got to be... centering. If imagefromset_alignClasses are defined,
	...
	function setDimensions( widget ) {
		var data = widget.data,
			...
			pageWidth = widget.editor.config.imagefromset_pageWidth;

		if (data.width > pageWidth/2) {
			//image.removeAttribute( 'height' );
			
			if (data.width >= data.height) {
				image.setAttribute( 'width', Math.round(pageWidth/2 ));
				image.setAttribute( 'height', Math.round(pageWidth/2/data.width*data.height ));
			} else {
				image.setAttribute( 'width', Math.round(pageWidth/3 ));
				image.setAttribute( 'height', Math.round(pageWidth/3/data.width*data.height ));
			}
		} else {
			for ( var d in dimensions ) {
				if ( dimensions[ d ] )
	...
	function setupResizer( widget ) {
		...
		resizer.setAttribute( 'title', editor.lang.imagefromset.resizer );
	...
	// on configurations like config#imagefromset_alignClasses or
	// config#imagefromset_captionedClass.
	//
	// @param {CKEDITOR.editor}
	// @returns {Object}
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.imagefromset_alignClasses,
			rules = {
				...
				figure: {
					classes: '!' + editor.config.imagefromset_captionedClass
				},
				figcaption: true
			};
 ...
 *		CKEDITOR.config.imagefromset_captionedClass = 'captionedImage';
 *
 * @cfg {String} [imagefromset_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.imagefromset_captionedClass = 'ag-capt-image';

if (!CKEDITOR.config.imagefromset_pageWidth) {
	CKEDITOR.config.imagefromset_pageWidth = 920;
}
if (!CKEDITOR.config.imagefromset_imageSet) {
	CKEDITOR.config.imagefromset_imageSet = [['Es gibt keine Bilder','']];
}
 ...
 *
 *		config.imagefromset_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4
 * @cfg {String[]} [imagefromset_alignClasses=null]
 * @member CKEDITOR.config
 */
